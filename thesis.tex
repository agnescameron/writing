...
\begin{marginfigure}
\includegraphics[width=\textwidth]{img/1/the-founder.jpg}
\caption{Nicky Case's \emph{Parable of the Polygons} creates an explorable version of Schelling's segregation models, allowing the player to modify the parameters of the simulation \label{b} \cite{tseng_founder_2017-1}}
\end{marginfigure}
...
There are 3 recycling bins in the group space (compared to 7 trash bins), two in the main shared work area (one large, and one small), and one in a separate meeting room that is used occasionally by people from outside of the group. The measured contamination rate before the study is consisent (within the margin of error) with the contamination rate measured during the last lab-wide waste audit, which was around 25\% of the lab's total recycling. The error calculated is the Standard Mean Error (standard deviation/sqrt(number of data points)).

\begin{figure}
  \caption{Chart showing the change in average contamination rate across the duration of the study. The first 2 columns show aggregate data from before and after the study, with the latter two columns showing the change in average contaminant over the time after the study. Error bars are shown in black.}
  \includegraphics[width=1\linewidth]{img/4/contamination-error.png}
  \label{contamination}
\end{figure}
...
In response to the question `What actions do you think are important in making MIT's waste systems more sustainable?', responses before and after both citing a lack of awareness and coherent signage, with two answers after the game specifically referencing recycling contamination. This is an infrastructural issue that has been a key focus of the Zero Waste Pilot Scheme.
...
\subsection*{Analysis}
The amount that was recycled on various days was subject to high levels of variation, with fluctuations according to the people using the space on a particular day, and the materials passing through. When averaged over the periods before and after the study, there is a decrease in the average percentage of contaminants of 12.5\% in the weeks directly before and after the study, shown in figure \ref{contamination}. However, this is subject to a large margin of error: for example, the day with the greatest percentage contamination occurred after the study, with a small number of items recycled, of which 3 were contaminants. The contamination rate also increases after the first week, implying that potential behavioural changes decay over time. There was no major change observed in the nature of the contaminants, with items identified as contaminants within the game (coffee cups, plastic bags), appearing in the waste streams both before and after the study (though at a slightly reduced rate). The resultant data do not show a conclusive behavioural change within the margins of error.
...
This thesis contributes 2 civic games that explore and analyse waste systems. The first, a card game, uses principles of transgressive behaviour in game design to re-imagine a `recycling sorting' game, and highlights the importance of speciality streams. The second, a simulation game modelled on waste systems at MIT makes legible the `complex information system' that is campus waste disposal, and demonstrates the effects of particular behaviours to stakeholders within the system.

This thesis incorporates a study that uses mixed methods to observe links between action and behaviour, and observes limitations in the efficacy of games to change a relationship with a system that is necessarily grounded in habit, and does not otherwise change in relation to the subject. This result is one discussed by a number of scholars of environmental psychology, but it contradicts a popular belief: that awareness is sufficient to shape forces of habit.
